...
function extractMessagesInOrder(conversation) {
  ...
  // ChatGPT stores edited/regenerated replies as sibling branches. The
  // selected branch is identified by current_node; blindly following the
  // first child can stop at an abandoned branch (notably in Pro chats).
  // Reconstruct the active branch from the leaf back to the root instead.
  if (conversation.current_node && mapping[conversation.current_node]) {
    ...
    while (nodeId && mapping[nodeId] && !visited.has(nodeId)) {
      ...
      if (node.message && node.message.content) branch.push(node.message);
      nodeId = node.parent;
    }
...
function rewriteSandboxFileLinks(text, metadata) {
  if (!CONFIG.downloadFiles || typeof text !== 'string') return text;

  const references = metadata?.content_references;
  ...
  for (const reference of references) {
    ...
    const ext = path.extname(filename) || mimeToExtension(reference.mime_type || reference.content_type);
    const localFileName = path.extname(fileId) ? fileId : `${fileId}${ext}`;
    const localUrl = `files/${encodeURIComponent(localFileName)}`;

    // Work deliverables are exposed in message text as ephemeral sandbox:
    // URLs while their durable file IDs live in content_references.
    for (const suffix of new Set([filename, encodeURI(filename)])) {
      const sandboxUrl = new RegExp(`sandbox:/[^\\r\\n)]*/${escapeRegExp(suffix)}`, 'g');
      rewritten = rewritten.replace(sandboxUrl, localUrl);
    }
  }
...
function extractMessageContent(message) {
  ...
  if (content.content_type === 'text' && content.parts) {
    ...
    return rewriteSandboxFileLinks(text, metadata);
  }
  ...
  if (content.content_type === 'multimodal_text' && content.parts) {
    ...
    for (const part of content.parts) {
      if (typeof part === 'string') {
        ...
        if (CONFIG.downloadFiles && fileId) {
          ...
          // Encode the path segment so composite asset IDs containing "#"
          // are linked as filenames rather than interpreted as URL fragments.
          const localFileName = path.extname(fileId) ? fileId : `${fileId}${ext}`;
          parts.push(`![image](files/${encodeURIComponent(localFileName)})`);
        } else if (fileId) {
    ...
    return rewriteSandboxFileLinks(parts.join('\n'), metadata);
  }
