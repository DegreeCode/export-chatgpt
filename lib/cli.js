...
function normalizeSessionCookie(input) {
  ...
  if (cookieHeaderLine) {
    // Also accept a copied Request Headers block and keep only its Cookie row.
    cleaned = cookieHeaderLine.replace(/^cookie\s*:\s*/i, '');
  } else if (lines.length >= 2 && /^cookie$/i.test(lines[0])) {
    // Chrome may copy the header name and value as two separate lines.
    cleaned = lines[1];
  } else {
    cleaned = text.replace(/^cookie\s*:\s*/i, '');
  }
  ...
  // Fetch/undici requires request-header values to be ByteStrings. Browser
  // cookies are ASCII (non-ASCII cookie data is percent-encoded), so Korean
  // UI labels here mean the user copied more than the Cookie value.
  for (let i = 0; i < cleaned.length; i++) {
    const code = cleaned.charCodeAt(i);
    if (code < 0x20 || code > 0x7e) {
      const hex = code.toString(16).toUpperCase().padStart(4, '0');
      throw new Error(`Invalid CHATGPT_SESSION_COOKIE: non-ASCII character at index ${i} (U+${hex}). Copy only the Cookie request-header value.`);
    }
  }

  if (!cleaned) {
    throw new Error('Invalid CHATGPT_SESSION_COOKIE: the Cookie value is empty.');
  }
  return cleaned.includes('=')
    ? cleaned
    : `__Secure-next-auth.session-token=${cleaned}`;
}
...
function setupCLI() {
  ...
  program
    ...
    .option('--session-cookie <cookie>', 'Browser Cookie header for Library file downloads (or set CHATGPT_SESSION_COOKIE)')
    ...
    .option('--include-library', 'Also export your owned ChatGPT Library folders, files, and file versions', false)
    .option('--library-only', 'Only export your owned ChatGPT Library (skip conversations and projects)', false)
    ...
    .option('--retry-failed-files', 'Retry file IDs previously recorded as permanently failed. Without this flag, known failures remain skipped.', false)
...
Examples:
  ...
  # Library files that also require the browser session cookie:
  export-chatgpt --bearer "eyJ..." --include-library --session-cookie "__Secure-authjs.session-token.0=...; ..."

  # Export only Library folders, metadata, and every available file version:
  export-chatgpt --bearer "eyJ..." --library-only
...
`);
  ...
  const sessionCookie = opts.sessionCookie || process.env.CHATGPT_SESSION_COOKIE || null;

  return { opts, bearerToken, sessionToken, sessionCookie };
}

async function main() {
  let { opts, bearerToken, sessionToken, sessionCookie } = setupCLI();
  ...
  CONFIG.retryFailedFiles = !!opts.retryFailedFiles;
  ...
  CONFIG.includeLibrary = !!opts.includeLibrary || !!opts.libraryOnly;
  CONFIG.libraryOnly = !!opts.libraryOnly;
  if (CONFIG.libraryOnly) {
  ...
  try {
    if (sessionCookie) {
      CONFIG.sessionCookie = normalizeSessionCookie(sessionCookie);
    } else if (sessionToken) {
      // --token already represents the legacy NextAuth session cookie and can
      // therefore also authenticate Library content requests.
      CONFIG.sessionCookie = normalizeSessionCookie(sessionToken);
    }
  } catch (error) {
  ...
  // Resolve the authenticated user even when --no-user-dir is used. Project
  // listings can include conversations created by other members in shared
  // projects, so downstream filtering must not depend on directory nesting.
...
module.exports = { main, normalizeSessionCookie };
