...
function normalizeWorkspaceUserId(userId) {
  ...
  // Business project APIs may append "__<workspace-id>" to the base user ID.
  return userId.split('__', 1)[0];
}

function isConversationOwnedByCurrentUser(conversation) {
  const currentUserId = normalizeWorkspaceUserId(CONFIG.currentUserId);
  const ownerUserId = normalizeWorkspaceUserId(conversation?.owner?.user_id);

  // Fail closed: a project conversation without trustworthy ownership
  // metadata must never be treated as belonging to the authenticated user.
  return Boolean(currentUserId && ownerUserId && currentUserId === ownerUserId);
}

function keepOnlyOwnedProjectConversations(conversations, projectName) {
  const owned = conversations.filter(isConversationOwnedByCurrentUser);
  const skipped = conversations.length - owned.length;
  if (skipped > 0) {
    console.log(`  Skipped ${skipped} shared or unknown-owner conversation(s) in project "${projectName}".`);
  }
  return owned;
}
...
async function fetchProjectConversations(accessToken, project, progress) {
  ...
  if (fs.existsSync(projectConvIndexFile)) {
    try {
      ...
      const unfilteredCount = conversations.length;
      conversations = keepOnlyOwnedProjectConversations(conversations, project.name);
      if (conversations.length !== unfilteredCount) {
  ...
  while (hasMore) {
    ...
    try {
      ...
      if (data.items && data.items.length > 0) {
        const ownedItems = keepOnlyOwnedProjectConversations(data.items, project.name);
        for (const conv of ownedItems) {
