...
function loadProgress() {
  if (fs.existsSync(PATHS.progressFile)) {
    try {
      ...
      if (data.libraryIndexingComplete === undefined) data.libraryIndexingComplete = false;
      if (data.libraryDownloadComplete === undefined) data.libraryDownloadComplete = false;
      if (data.libraryLastCursor === undefined) data.libraryLastCursor = null;
      if (!data.libraryDownloadedFileIds) data.libraryDownloadedFileIds = [];
      if (!data.libraryFailedFileIds) data.libraryFailedFileIds = {};
      return data;
    } catch (e) {
