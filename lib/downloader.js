...
function extractFileReferences(conversationData) {
  ...
  for (const node of Object.values(conversationData.mapping)) {
    ...
    // Pro and Work responses expose cited/generated files in
    // metadata.content_references. Work deliverables additionally carry a
    // library_file_id and an ephemeral sandbox: link in the message text.
    // The per-reference file ID is still the resolver key and may differ for
    // each version of the same Library file, so deduplicate by file ID below.
    if (Array.isArray(node.message.metadata?.content_references)) {
      for (const reference of node.message.metadata.content_references) {
        if (!reference || reference.type !== 'file') continue;
        const fileId = reference.id || reference.file_id;
        if (!fileId) continue;

        const mimeType = reference.mime_type || reference.content_type || '';
        const filename = reference.name || reference.filename || '';
        const isImage = mimeType.toLowerCase().startsWith('image/') ||
          ['.png', '.jpg', '.jpeg', '.gif', '.webp', '.svg'].includes(path.extname(filename).toLowerCase());
        const isCanvas = reference.library_artifact_type === 'canvas';
        ...
        files.push({
          ...
          filename,
          mimeType,
          libraryFileId: reference.library_file_id || undefined,
          source: reference.source || undefined,
          metadata: reference,
          sizeBytes: reference.size || reference.size_bytes,
        });
      }
    }
  }
  ...
  for (const ref of files) {
    ...
    // A file can appear as both an asset/attachment and a content reference.
    // Merge the records so Library IDs and original filenames are retained
    // without downgrading an image/canvas classification to an attachment.
    const type = existing.type === 'image' || ref.type === 'image'
      ? 'image'
      : (existing.type === 'canvas' || ref.type === 'canvas' ? 'canvas' : 'attachment');
    deduplicated.set(ref.fileId, {
      ...existing,
      ...ref,
      type,
      filename: ref.filename || existing.filename,
      mimeType: ref.mimeType || existing.mimeType,
      libraryFileId: ref.libraryFileId || existing.libraryFileId,
      source: ref.source || existing.source,
      sizeBytes: ref.sizeBytes || existing.sizeBytes,
      metadata: { ...(existing.metadata || {}), ...(ref.metadata || {}) },
    });
  }
...
async function retryPendingFiles(accessToken, progress) {
  ...
  // Resolver v4 also discovers Pro/Work files stored in content_references.
  // Resolver v3 added /backend-api/files/{file_id}/download, used by
