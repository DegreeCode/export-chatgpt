...
async function getFileDownloadUrl(accessToken, fileId, conversationId) {
  ...
  for (let i = 0; i < urls.length; i++) {
    try {
      ...
      // Current ChatGPT accounts use both resolver routes. If the legacy
      // route returns a structured not-found response, try the conversation
      // attachment route before treating the file as unavailable.
      if (i === 0 && data?.status !== 'success' && data?.error_code === 'file_not_found') {
        continue;
      }
      return data;
    } catch (error) {
      lastError = error;
      if (i === 0 && /HTTP 404/.test(error.message)) continue;
      throw error;
    }
  }
...
async function retryPendingFiles(accessToken, progress) {
  ...
  // Resolver v2 adds the conversation attachment endpoint and support for
  // authenticated root-relative Library URLs. Reopen failures recorded by
  // the older resolver exactly once so they benefit from the new routes.
  const resolverVersion = 2;
  if ((progress.fileResolverVersion || 0) < resolverVersion) {
    ...
    if (reopenedFailures > 0) {
      console.log(`\nReopened ${reopenedFailures} file failure record(s) for resolver v${resolverVersion}.`);
    }
  }
