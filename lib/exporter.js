...
const { exportLibrary } = require('./library');
...
async function run(accessToken) {
  ...
  if (CONFIG.includeLibrary) {
    console.log(`Library export: ${CONFIG.libraryOnly ? 'Library only' : 'included'} (owned items, all versions)`);
  }
  if (CONFIG.downloadFiles) {
  ...
  try {
    if (!CONFIG.projectsOnly && !CONFIG.libraryOnly) {
      console.log('=== Regular Conversations ===\n');
      summary.regular = await exportConversations(accessToken, progress);
    }

    if (!CONFIG.libraryOnly && (CONFIG.includeProjects || CONFIG.projectsOnly)) {
    ...
    if (CONFIG.includeLibrary) {
      console.log('\n=== ChatGPT Library ===\n');
      summary.library = await exportLibrary(accessToken, progress);
    }

    if (CONFIG.downloadFiles && !CONFIG.libraryOnly) {
      const retried = await retryPendingFiles(accessToken, progress);
      if (retried > 0) summary.retriedFiles = retried;
      const failedCount = Object.keys(progress.failedFileIds).length + (progress.retryFailureCount || 0);
      if (failedCount > 0) summary.failedFiles = failedCount;
    }
  } catch (error) {
    if (error.authError) {
      summary.interrupted = true;
      summary.failedFiles = Object.keys(progress.failedFileIds).length;
...
function printSummary(summary) {
  ...
  console.log('\n' + '='.repeat(50));
  console.log(summary.interrupted ? '  Export Incomplete — authentication expired' : '  Export Complete!');
  ...
  if (summary.library) {
    ...
    const parts = [`${lib.files} files`, `${lib.directories} folders`, `${lib.versions} versions`];
    if (lib.downloaded > 0) parts.push(`${lib.downloaded} downloaded`);
    if (lib.reused > 0) parts.push(`${lib.reused} reused`);
    if (lib.existing > 0) parts.push(`${lib.existing} already present`);
    if (lib.filtered > 0) parts.push(`${lib.filtered} excluded by file-type flags`);
    if (lib.failed > 0) parts.push(`${lib.failed} failed`);
    if (lib.sharedSkipped > 0) parts.push(`${lib.sharedSkipped} shared/unknown skipped`);
    console.log(`  Library:        ${parts.join('    ')}`);
  }
